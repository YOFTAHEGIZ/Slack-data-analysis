...
class SlackDataLoader:
    ...
    def slack_parser(self, channel_name):
    ...
    def create_dataframe(self):
        channel_names = [self.channels[i]['name'] for i in range(len(self.channels))]
        data_frames = []

        for name in channel_names:
            channel_path = self.path + name + '/'
            channel_dataframe = self.slack_parser(name)
            data_frames.append(channel_dataframe)
        
        all_data = pd.concat(data_frames, ignore_index=True)
        return all_data
    def create_reaction_data_frame(self):
        channel_names = [self.channels[i]['name'] for i in range(len(self.channels))]
        reaction_data_frames = []

        for name in channel_names:
            channel_path = self.path + name + '/'
            channel_dataframe = self.parse_slack_reaction(name)
    ...
    def create_mention_data_frames(self, data):
        user_ids = utils.get_tagged_users(data)
        ids_list = []
        for id in user_ids:
            if id:
                ids_list.append(id[0][1:])
    
        all_userids_mentioned = pd.DataFrame({'user_id': ids_list})
        user_names_df = self.create_users_df()
